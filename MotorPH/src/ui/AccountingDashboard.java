...
public class AccountingDashboard extends JFrame {
    ...
    private JComboBox<String> cbMonth; // Declare here
...
btnCompute.addActionListener(e -> {
    // 1. UI Rule: Handle User Input (Table Selection)
    int row = empTable.getSelectedRow();
    
    if (row == -1) {
        showError("Please select an employee from the list first.");
        return;
    }

    try {
        // 2. Extract the ID from the first column (Column 0: 10001)
        int id = (int) empTable.getValueAt(row, 0);
        String selectedMonth = (String) cbMonth.getSelectedItem();

        // 3. UI calls Service: Service handles the DAO fresh fetch and Business Logic
        Deduction results = payrollService.calculateMonthlyPayroll(id, selectedMonth);

        // 4. UI Rule: Handle "Front-end Ichura" (Update labels)
        // We fetch the employee one last time just to show the basic info labels
        Employee selectedEmp = employeeDao.findById(id);
        
        if (selectedEmp != null) {
            updatePayslipUI(selectedEmp, results, results.getLateMinutes());
            
            // Success feedback
            System.out.println("Payroll computed for: " + selectedEmp.getFirstName());
        } else {
            showError("Could not find employee data for ID: " + id);
        }
   ...
   private void calculateAndDisplayPayroll(Employee emp, String month) {
    if (emp == null) return;
    // Calls the Service Layer method that accepts (int, String)
    Deduction d = payrollService.calculateMonthlyPayroll(emp.getEmpNo(), month);
...
// Inside your UI Class (AccountingDashboard)
private void handleComputePayroll() {
    try {
        int id = Integer.parseInt(txtEmpNo.getText());
        String month = (String) cbMonth.getSelectedItem();

        // UI CALLS SERVICE
        Deduction results = payrollService.calculateMonthlyPayroll(id, month);

        // UI DISPLAYS MODEL DATA
        lblSSS.setText(String.format("₱%.2f", results.getSss()));
        lblNetPay.setText(String.format("₱%.2f", results.getNetPay()));
        
        if (results.getNetPay() <= 0) {
            JOptionPane.showMessageDialog(this, "Check Employee Salary in Database.");
        }
    } catch (Exception e) {
        showError("Please select a valid employee.");
    }
}

// Add this inside AccountingDashboard.java
private void showError(String msg) {
    JOptionPane.showMessageDialog(this, msg, "MotorPH Error", JOptionPane.ERROR_MESSAGE);
}
