...
public class CSVHandler implements EmployeeDAO {
   ...
   private void loadAllIntoCache() {
    ...
    // 1. Load Data from MotorPH_EmployeeData.csv (Base Data)
    try (BufferedReader br = new BufferedReader(new FileReader(EMPLOYEE_DATA_CSV))) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            String[] data = CSVUtils.splitCSVLine(line);
            
            // Clean Byte Order Mark (BOM) and whitespace from the first column
            if (data.length > 0 && data[0] != null) {
                data[0] = data[0].replace("\uFEFF", "").trim();
            }

            // Ensure we have enough columns for basic mapping
            ...
            if (!bdayStr.isEmpty()) {
                try { 
                    ...
                    // Silent catch for invalid date formats
                }
            }
            
            // DAO maps raw file data to a RegularStaff model initially
        ...
        System.err.println("Employee File Error: " + e.getMessage());
    }

    // 2. Load Credentials and Upgrade Roles from MotorPH_EmployeeLogin.csv
    try (BufferedReader br = new BufferedReader(new FileReader(LOGIN_DATA_CSV))) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (loginData.length >= 6) {
                try {
                    // Clean ID string from Login CSV
                    String rawId = loginData[0].replace("\uFEFF", "").trim();
                    ...
                    if (baseEmp != null) {
                        ...
                        String roleStr = loginData[5].trim(); // e.g., "Accounting"
                        
                        // DAO Logic: Convert base Employee to specialized subclass (Admin, AccountingStaff, etc.)
                        Employee upgraded = upgradeEmployeeRole(baseEmp, roleStr);
                        
                        // Set credentials
                        upgraded.setPassword(password);
                        
                        // CRITICAL FIX: Ensure the "Position" field is explicitly set 
                        // so the Service Layer (UserLibrary) can determine the Role later
                        upgraded.setPosition(roleStr); 
                        
                        // Save specialized object back to cache
  ...
  private Employee mapToEmployee(String[] data, LocalDate birthday) {
    try {
        int id = Integer.parseInt(CSVUtils.clean(data[0])); // Column 0
        
        // --- CORRECTED MAPPING ---
        double basicSalary = CSVUtils.parseCurrency(data[13]); // Column 13: 90000.00
        
        Employee emp = new RegularStaff(id, data[1], data[2], birthday, basicSalary);
        
        // Personal Details
        emp.setPosition(CSVUtils.clean(data[11]));    // Column 11: CEO
        emp.setSupervisor(CSVUtils.clean(data[12]));  // Column 12: N/A
