...
public class ITDashboard extends JFrame {
    ...
    public ITDashboard(EmployeeDAO dao, Employee user) {
        ...
        // Load data for the Home/Profile view
        loadEmployeeDetails();
    }
...
private JPanel createHomePanel() {
   ...
   Runnable refresh = () -> {
    ...
    private JPanel createTablePanel() {
        ...
        return new JPanel(new BorderLayout()) {{ add(new JScrollPane(userTable), BorderLayout.CENTER); }};
    }

    private JPanel createBottomForm() {
        JPanel form = createGridPanel("Ticket Detail View", 2, 4);
        // Shared fields used for IT lookup
        return form;
    }
    ...
    btnReset.addActionListener(e -> {
        try {
            int id = Integer.parseInt(txtEmpNo.getText());
            String newPass = JOptionPane.showInputDialog(this, "Enter Temporary Password:");
            
            if (newPass != null && !newPass.trim().isEmpty()) {
                // Call the SERVICE
                boolean success = itService.resetCredentials(id, newPass);
                
                if (success) {
                    JOptionPane.showMessageDialog(this, "Credentials Reset & Account Reactivated!");
                    refreshTableData("ALL");
                }
            }
        } catch (NumberFormatException ex) {
